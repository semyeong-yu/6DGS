...
class GaussianModel:
    ...
    def densify_and_split(self, scale, grads, grad_threshold, scene_extent, N=2):
    ...
    def densify_and_clone(self, scale, grads, grad_threshold, scene_extent):
        # Extract points that satisfy the gradient condition
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, radii):
        cov_cond = self.get_covariance(self.get_scaling, self.get_rotation)[0] # (N, 6)
        scale_temp, rotation_temp = self.extract_SR(cov_cond) # (N, 3, 3), (N, 3, 3)
        scale, _ = self.convert_matrix_vector(scale_temp, rotation_temp) # (N, 3), (N, 4)

        grads = self.xyz_gradient_accum / self.denom # do not reflect self.normal_gradient_accum into grads
        grads[grads.isnan()] = 0.0

        self.tmp_radii = radii
        self.densify_and_clone(scale, grads, max_grad, extent)
        self.densify_and_split(scale, grads, max_grad, extent)
